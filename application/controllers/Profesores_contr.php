...
class Profesores_contr extends CI_Controller {
	...
	{
		parent::__construct();		
		$this->load->model('Profesores_model','prof');
	...
	public function vista()
	{
		...
		$this->load->view('Profesores_view');
	...
	public function ajax_set_profe()
	{
		...
		$data=array(
			...
			'activo'=>$this->input->post('activado'),
		);
		//print_r($data);
	...
	public function ajax_list()
	{
		...
		foreach ($list as $profesor) {
			...
			//add html for action
			$row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_profesor('."'".$profesor->id_prof."'".')"><i class="glyphicon glyphicon-pencil"></i> Edit</a>
				<a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_profesor('."'".$profesor->id_prof."'".')"><i class="glyphicon glyphicon-trash"></i> Eliminar</a>';
	...
	public function ajax_update_profe()
	{
		...
		$this->prof->update(array('id_prof'=>$this->input->post('id')),$data);
