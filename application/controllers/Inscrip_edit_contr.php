...
class Inscrip_edit_contr extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Inscrip_edit_model','estud');
	...
	public function vista()
	{
		...
		$this->load->view('inscrip_edit_view');
	...
	public function getstuden()
	{
		...
		$list=$this->estud->get_data_estudiantes1($id);
	...
	public function getpandemia() {
		$id=$this->input->post('idest');
		$list=$this->estud->get_data_pandemia($id);
		$data=array();
		$data[]=$list->clases;
		$data[]=$list->vacunas;
		$output=array(
	...
	/*public function getinscripcion() {
		$id=$this->input->post('idest');
		$gestion=$this->input->post('gestion');
		$list=$this->estud->get_data_inscripcion($id, $gestion);
		$data=array();
		$data[]=$list->clases;
		$data[]=$list->vacunas;
		$output=array(
	...
	public function ajax_save_estud()
	{
		...
		// $idins=$this->ajax_get_id();
		// $idest=$this->ajax_get_idestnewA();
		// $ctto=$this->ajax_get_numcttoA();

		// $data0=array(
		// 	'idinscrip'=>$idins,
		// 	'idest'=>$this->input->post('idestnew'),
		// 	'usuario'=>$this->input->post('usuario'),
		// 	'fecha'=>$this->input->post('fechains')
		// );
		// $insert0=$this->estud->save_usuario($data0);
		

		
		// $data1=array(
		// 	'unidedu'=>$this->input->post('unidedu'),
		// 	'depend'=>$this->input->post('depend'),
		// 	'distrito'=>$this->input->post('distrito'),
		// 	'sie'=>$this->input->post('sie'),			
		// 	'idinscrip'=>$idins
		// );
		// $insert1=$this->estud->save_unidadeduca($data1);
		...
		// 	'idcurso'=>$this->input->post('idcurnew'),
		// 	'nivel'=>$this->input->post('nivel'),
		// 	'gestion'=>$this->input->post('gestion'),
		...
		// );
		// $insert2=$this->estud->save_estudiante($data2);

		// echo "string".$this->input->post('contrato');
		// exit();
		// print_r("expression");
		// exit();
		...
		$incripccion1=$this->estud->updates("reguistro_incripccion",$where,$incripccion);
		...
		$factura1=$this->estud->updates("reguistro_factura",$where,$factura);

		$pandemia = array(
			'clases' => $this->input->post('pandemia_clases'),
			'vacunas' => $this->input->post('pandemia_vacunas')
		);
		$pandemia1 = $this->estud->updates("reguistro_pandemia", array('id_est'=>$id_est), $pandemia);
		...
		$antcolegio1=$this->estud->updates("reguistro_colegio",$where,$antcolegio);
		...
		$direccion1=$this->estud->updates("reguistro_direccion",$where,$direccion);
		...
		$salud1=$this->estud->updates("reguistro_salud",$where,$salud);
		...
		$servicios_basico1=$this->estud->updates("reguistro_servicios_basicos",$where,$servicios_basico);
		...
		$transporte1=$this->estud->updates("reguistro_transporte",$where,$transporte);
		...
		$trabajo1=$this->estud->updates("reguistro_trabajo",$where,$trabajo);
		...
		$nota_prom=array(
			'codigo'=>$this->input->post('curso')."-".$this->input->post('nivel')."-".$this->input->post('unidedu')
		);
		$nota_prom1=$this->estud->updates("nota_prom",$where,$nota_prom);
		...
		if($this->input->post('t2id')==0)
		{
			if($this->input->post('t2ci')!=null)
			{
				...
				$reg_tutor3=$this->estud->updates("reguistro_tutor",$wheremadre,$wheremadre1);
			}
		}else{
			$fecham=$this->input->post('t2fn');
			$wheremadre=array(
			...
			$madre1=$this->estud->updates("padres",$wheremadre,$madre);
		...
		if($this->input->post('t3id')==0)
		{
			if($this->input->post('t3ci')!=null)
			{
				...
				$reg_tutor3=$this->estud->updates("reguistro_tutor",$wheretutor,$wheretutor1);
			}
		}else{
			$fechat=$this->input->post('t3fn');
			$wheretutor=array(
			...
			$tutor1=$this->estud->updates("padres",$wheretutor,$tutor);
		...
		if($this->input->post('contrato')=="P")
		{
			...
			$contrato1=$this->estud->updates("reguistro_contrato",$wherepa,$contrato);
		}
  ...
  //       	$con=$con+4;;
 	// 	}
		...
		// $estudiante=$this->estud->get_data_estudiantes($nacimientos->idest);
		$estudiantes=$this->estud->get_data_estudiantes1($id_est);
	...
	public function print_ctto($cod)
	{
		...
		ob_start();
			...
			$this->pdf->SetTitle("Contrato");
				...
				$this->pdf->Cell(30);
            	$this->pdf->Cell(135,8,utf8_decode('CONTRATO PRIVADO DE SERVICIO EDUCATIVO 2020'),0,0,'C');
            	...
            	$this->pdf->setXY(113,22);$this->pdf->SetFont('Arial','',5);
            	...
            	$this->pdf->Cell(30,8,utf8_decode($fechas[2]),0,0,'L');
            	$this->pdf->setXY(90,218);
            	$this->pdf->Cell(30,8,utf8_decode($fechas[1]),0,0,'L');
...
DÉCIMA. Nosotros PADRE DE FAMILIA y UNIDAD EDUCATIVA, manifestamos nuestra absoluta conformidad con todas y cada una de las cláusulas precedentes, por lo que suscribimos para su fiel y estricto cumplimiento, en la ciudad de Sucre a los ............ días del mes  ....................... del año 2020.
'),0);
        ...
        {	
            	...
            	$this->pdf->Cell(50,8,utf8_decode($curso),0,0,'R');
            	$this->pdf->setXY(113,45);$this->pdf->SetFont('Arial','',5);
            	$this->pdf->Cell(50,8,utf8_decode($nivel),0,0,'R');
            	$this->pdf->SetFont('Arial','',6);
            	$this->pdf->setXY(55,49);
            	$this->pdf->Cell(50,8,utf8_decode($calle),0,0,'L');
            	...
            	$this->pdf->MultiCell(180,4,utf8_decode('Conste por el presente documento de manifestación de voluntades, que establecen acuerdos para llevar adelante la gestión escolar 2020 de la Unidad Educativa de Convenio Técnico Humanístico Don Bosco "A"–"B" EPDB, turno tarde, de acuerdo a las cláusulas siguientes:
PRIMERA.- DE LAS PARTES CONSENSUANTES.- Intervienen en el presente convenio por una parte la Unidad Educativa de Convenio Técnico Humanístico Don Bosco "A"-"B", de la ciudad de Sucre, administrada a la Sociedad San Francisco de Sales (SFS), con Resolución Administrativa No 641/2012 -642/2012  SIE 80480192 - 80480261 "A"-"B" representada legalmente por su Director General el Padre Alberto Vásquez Cáceres sdb, con C.I. 3122876 Cbba. Quien cuenta con las facultades suficientes de acuerdo a mandato que parte adjunta e indisoluble del presente acuerdo, para fines del presente documento en adelante se denominará UNIDAD EDUCATIVA.  
...
El Ministerio de Educación, como ente rector del sector de la educación en el país, emite en fecha 02 de enero del 2020, la Resolución Ministerial No 001/2020 "NORMAS GENERALES PARA LA GESTION EDUCATIVA Y ESCOLAR  2020" la cual constituye la base legal del presente Convenio. 
TERCERA.- Por el presente documento ambas partes UNIDAD EDUCATIVA y PADRE DE FAMILIA  acuerdan que la UNIDAD EDUCATIVA se compromete a otorgar al estudiante durante la gestión escolar 2020 en el turno de la Tarde una Enseñanza - Aprendizaje, de calidad y eficiencia basados en los programas oficiales del Estado Plurinacional de Bolivia y aplicando sistemas modernos, teniendo como pilar fundamental la educación cristiana acorde a la moral y las normas de la Iglesia Católica, poniendo en práctica el Sistema Preventivo de Don Bosco, formación en valores morales y respetando la libertad religiosa y de culto. 
