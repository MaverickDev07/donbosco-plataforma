...
class Estudiantes_pe_contr extends CI_Controller {
	...
	{
		parent::__construct();		
		$this->load->model('Estudiantes_pe_model','mat');
	...
	public function vista()
	{
		...
		$this->load->view('Estudiantes_pe_view');
	...
	public function ajax_set_pregunta()
	{ 
		...
		//print_r($data);
		//exit();
		$insert=$this->mat->save_pregunta($data);
	...
	{	
		...
		foreach ($list as $temas) {
			...
			$row[] = $temas->tema;
			$row[] = $temas->pregunta;
			if($temas->id_est==$id_est){
				$row[] = '<a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_pregunta('."'".$temas->id."'".')"><i class="glyphicon glyphicon-trash"></i> Eliminar</a>';
			}else{
			$row[] ="-";
}		
			//add html for action
	...
	public function ajax_delete_pregunta($id)
	{
		$this->mat->delete_by_id($id,"pregunta");
		echo json_encode(array("status"=>TRUE));
	}
