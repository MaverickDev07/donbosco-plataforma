...
class Est_Inscrip_contr extends CI_Controller {
	...
	public function ajax_save_estud()
	{
		...
		$incripccion2=array(
			'fecha'=>$fecha,
			'gestion'=>$this->input->post('gestion')+1,
			'vive'=>$this->input->post('vivecon'),
			'id_est'=>$id_est,
			'id_user'=>$id_est,
			'curso'=>$cursos1."".$cursos[1],
			'colegio'=>$colegio,
			'nivel'=>$nivel
		);
		$where=array(
		...
		exit();*/
		$nota_prom=array(
			'codigo'=>$cursos1."".$cursos[1]."-".$nivel."-".$colegio,
			'id_est'=>$id_est,
			"vive"=>$this->input->post('vivecon'),
			'gestion'=>$this->input->post('gestion')+1
		);
		...
		$numero1=$this->estud->count_contrato($this->input->post('gestion')+1);
  ...
  //       	$con=$con+4;;
    ...
    $nombre=$padre->appaterno." ".$padre->apmaterno." ".$padre->nombre;
  	...
  	$estudiante1=$this->estud->get_data_estudiante($id_est);
		...
		if($turno=='MAÑANA') {
			if($cod_curso=='5A' | $cod_curso=='5B' | $cod_curso=='5C' | $cod_curso=='6A' | $cod_curso=='6B' | $cod_curso=='6C'  ) {
      	...
      	$this->pdf->SetFont('Arial','BU',8);
				$this->pdf->Cell(30);
        $this->pdf->Cell(135,8,utf8_decode('CONTRATO DE PRESTACION DE SERVICIOS EDUCATIVOS'),0,0,'C');
...
El Ministerio de Educación, como ente rector del sector de la educación en el país, emite en fecha....... de enero del 2021, la Resolución Ministerial No 001/2021 denominada "NORMAS GENERALES PARA LA GESTION EDUCATIVA 2021" la cual constituye la base legal de este Contrato de servicios educativos privados. 
...
CUARTA.- DEL PRECIO Y FORMA DE PAGO DE LOS SERVICIOS EDUCATIVOS, CONSTITUCIÓN DE MORA.- Por la prestación de servicios educativos EL  RESPONSABLE se obliga a cancelar a la UNIDAD EDUCATIVA un COSTO ANUAL de Bs. 6.901.- Pagaderos en 10 cuotas mensuales  de Bs. 691,50 hasta el día 10 de cada mes; sin perjuicio que el RESPONSABLE pueda cancelar lo adeudado en un solo pago o de manera anticipada. Los pagos deberán realizarse a la Cuenta Nº 4010765389 del Banco Mercantil Santa Cruz S.A. por los pagos recibidos la UNIDAD EDUCATIVA emitirá la factura correspondiente.
