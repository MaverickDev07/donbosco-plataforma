...
class Estudiantes_de_contr extends CI_Controller {
	...
	{
		parent::__construct();		
		$this->load->model('Estudiantes_de_model','mat');
	...
	public function vista()
	{
		...
		$this->load->view('Estudiantes_de_view');
	...
	{	
		...
		foreach ($list as $temas) {
			...
			if($categoria=="A"){
				$row[] ="TEMAS";
				$row[] = '<a class="btn btn-sm btn-primary"  title="Descargar " href="'.base_url().'Subir_tarea_contr/descarga_temas/'.$temas->id.'"><i class="glyphicon glyphicon-pencil"></i> Descargar </a>';
			}else{
				$row[] ="TAREAS";
				$row[] = '<a class="btn btn-sm btn-primary"  title="Descargar " href="'.base_url().'Subir_tarea_contr/descarga_tarea/'.$temas->id.'"><i class="glyphicon glyphicon-pencil"></i> Descargar </a>';
			}
	...
	public function descarga_temas($id)
	{
		...
		$tema=$this->mat->get_temas_nombre($id);
		//print_r($tema);
		//exit();
		$data=array(
			'id_est'=>$this->session->userdata("id"),
			...
			//print_r($nombre_carpeta);exit();
	$insert=$this->mat->insert($data,"bicacora_estudiante");
