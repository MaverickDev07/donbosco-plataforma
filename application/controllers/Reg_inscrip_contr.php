...
class Reg_inscrip_contr extends CI_Controller {
	public function __construct()
	{
		parent::__construct();		
		//$this->load->helper(array('url', 'form'));
		$this->load->helper('url');
		$this->load->model('Reg_inscrip_model','estud');
	...
	public function vista()
	{
		...
		$this->load->view('Reg_inscrip_view');
	...
	public function ajax_list_idcursos($id)
	{
		$list=$this->estud->get_datatables_by_idcurs();
		$data = array();
		$no = $_POST['start'];
		...
		foreach ($list as $estudiante) {
			...
			//add html for action
			$row[] = '<a class="btn btn-sm bg-orange-400" href="javascript:void(0)" title="Edit" onclick="inscrip('."'".$estudiante->id_est."'".')"><i class="glyphicon glyphicon-pencil"></i> Inscribir</a>';
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->estud->count_all(),
						"recordsFiltered" => $this->estud->count_filtered_byids(),
						"data" => $data,
				);
