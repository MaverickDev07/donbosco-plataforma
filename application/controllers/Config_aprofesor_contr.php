...
class Config_aprofesor_contr extends CI_Controller {
	...
	{
		parent::__construct();		
		$this->load->model('Config_aprofesor_model','mat');
	...
	public function vista()
	{
		...
		$this->load->view('config_aprofesor_view');
	...
	{ 
		...
		$curso=$this->mat->get_curso($curso);
		$ag_profesor=$this->mat->get_ag_profesor($gestion,$materia,$idprof);
		$estudiante=$this->mat->get_estudiantes($curso->codigo,$gestion,$nivel);
		$trimestre=$this->mat->get_trimestre();
		foreach ($estudiante as $estudiantes) {
			foreach ($trimestre as $trimestres) {
				$data1=array(
				'codigo'=>$ag_profesor->codigo,
				'gestion'=>$gestion,
				'id_asg_prof'=>$ag_profesor->id_asg_prof,
				'id_bi '=>$trimestres->id_bi,
				'id_est'=>$estudiantes->id_est			
				);
				$inserts=$this->mat->save($data1,"nota_bimestre");
			}
		}
	...
	{		
		...
		$list=$this->mat->get_datatables_gestion($curso,$nivel,$gestion);
		// print_r($list);
		// exit();
		$data = array();
		$no = $_POST['start'];
		...
		foreach ($list as $profesor) {
			...
			//add html for action
			$row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_profesor('."'".$profesor->id."'".')"><i class="glyphicon glyphicon-pencil"></i> Edit</a>
				<a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_materia('."'".$profesor->id."'".')"><i class="glyphicon glyphicon-trash"></i> Eliminar</a>';
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->mat->count_all(),
						"recordsFiltered" => $this->mat->count_filtered_gestion($curso,$nivel,$gestion),
						"data" => $data,
				);
