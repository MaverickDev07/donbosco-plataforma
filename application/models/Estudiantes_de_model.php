...
class Estudiantes_de_model extends CI_Model {
 
	var $tabla='temas';
	var $column= array('temas.id','materias.nombre','temas.tema','profesores.appaterno','profesores.apmaterno','profesores.nombre'); //set column field database for datatable orderable
	...
	{		
		$this->db->select('temas.id as id,materias.nombre as materia,temas.tema as tema,profesores.appaterno,profesores.apmaterno,profesores.nombre,temas.nombre as archivo');
		$this->db->from("temas");
		$this->db->join('profesores', 'profesores.id_prof = temas.id_prof','inner');
		$this->db->join('materias', 'materias.id_mat = temas.id_mat','inner');
		$this->db->where('temas.gestion',$gestion);
