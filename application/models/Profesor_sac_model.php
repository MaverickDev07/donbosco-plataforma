...
class Profesor_sac_model extends CI_Model {
 
	var $tabla='temas';  
	var $column= array('semana_academica.id','materias.nombre','temas.tema'); //set column field database for datatable orderable
	...
	var $order = array('semana_academica.dia'=> 'asc'); // default order 
	...
	private function _get_datatables_query_gestion($gestion,$nivel,$id_prof)
	{		
		$this->db->select('semana_academica.*,materias.nombre as materia,cursos.nombre as curso');
		$this->db->from("semana_academica");
		$this->db->join('materias', 'materias.id_mat = semana_academica.id_mat','inner');
		$this->db->join('cursos', 'cursos.codigo = semana_academica.cod_curso','inner');
		$this->db->where('semana_academica.gestion',$gestion);
		$this->db->where('semana_academica.cod_nivel',$nivel);
		$this->db->where('semana_academica.id_prof',$id_prof);
		$i = 0;
	...
	function get_datatables_gestion($gestion,$nivel,$id_prof)
	{
		$this->_get_datatables_query_gestion($gestion,$nivel,$id_prof);
		if($_POST['length'] != -1)
		$this->db->limit($_POST['length'], $_POST['start']);
		$query = $this->db->get();
		return $query->result();
	...
	public function rows_curso_materias($nivel,$curso,$materia,$gestion)
	{
		$this->db->select('asiginar_profesorm.id_asg_prof as id');
		$this->db->from("asiginar_materiacu");
		$this->db->join('asiginar_profesorm', 'asiginar_profesorm.id_asg_mate = asiginar_materiacu.id_asg_mate','inner');
		$cod=$curso."-".$nivel."-";
	...
	public function get_bitacora_estudiante($gestion,$id_tema)
	{
		$this->db->select('bicacora_estudiante.id as id,bicacora_estudiante.fecha,estudiantes.appaterno,estudiantes.apmaterno,estudiantes.nombre');
		$this->db->from("bicacora_estudiante");
		$this->db->join('temas', 'temas.id = bicacora_estudiante.id_tema','inner');
		$this->db->join('estudiantes', 'estudiantes.id_est = bicacora_estudiante.id_est','inner');
		$this->db->where('temas.gestion',$gestion);
	...
	function get_nota_contar($gestion,$id_prof,$id_est,$tipo)
	{
	...
	function get_nota_tareas($id_tarea)
	{
		$this->db->from("nota_tareas");
		$this->db->where('nota_tareas.id_tarea',$id_tarea);
		$query = $this->db->get();
		return $query->row();
	}
	...
	public function get_tema($id_tema)
	{
		$this->db->from("temas");
	...
	public function get_materia($id_mat)
	{
		$this->db->from("materias");
		$this->db->where('id_mat', $id_mat);
		$query=$this->db->get();
	...
	function count_filtered_gestion($gestion,$nivel,$id_prof)
	{
		$this->_get_datatables_query_gestion($gestion,$nivel,$id_prof);
		$query = $this->db->get();
		return $query->num_rows();
	...
	public function updates($where,$data,$tabla)
	{
