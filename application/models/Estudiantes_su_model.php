...
class Estudiantes_su_model extends CI_Model {
 
	var $tabla='temas';
	var $column= array('temas.id',"profesores.appaterno","profesores.apmaterno","profesores.nombre",'temas.gestion','materias.nombre','temas.tema'); //set column field database for datatable orderable
	...
	var $order = array('tareas.id_tema' => 'asc'); // default order 
	...
	{		
		...
		$this->db->join('tarea_subir', 'temas.id = tarea_subir.id_tema','inner');
...
/*$this->db->select('tarea_subir.*,temas.gestion,materias.nombre as materia,temas.tema as tema, DATE_FORMAT(tarea_subir.fecha, "%H:%I:%S" )as hora,Date_format(tarea_subir.fecha,"%Y/%M/%d") as fecha,profesores.appaterno,profesores.apmaterno,profesores.nombre,tarea_subir.nombre as archivo');
		$this->db->from("tarea_subir");
		$this->db->join('temas', 'temas.id = tarea_subir.id_tema','inner');
		$this->db->join('profesores', 'profesores.id_prof = temas.id_prof','inner');
		$this->db->join('materias', 'materias.id_mat = temas.id_mat','inner');
		...
		$this->db->where('temas.id_mat',$materia);
		$this->db->where('temas.id_prof',$id_prof);*/
	private function _get_datatables_query_gestion($cod_curso,$cod_nivel,$gestion,$materia,$id_prof)
	{		
		$this->db->select('tareas.id as id_tarea,temas.id as id_tema,temas.gestion,materias.nombre as materia,temas.tema as tema, Date_format(tareas.fechaentrega,"%Y/%M/%d") as fechaentrega, Date_format(tareas.fecha,"%Y/%M/%d") as fecha,profesores.appaterno,profesores.apmaterno,profesores.nombre,tareas.nombre as archivo');
		$this->db->from("tareas");
		$this->db->join('temas', 'temas.id = tareas.id_tema','inner');
		$this->db->join('profesores', 'profesores.id_prof = temas.id_prof','inner');
		$this->db->join('materias', 'materias.id_mat = temas.id_mat','inner');
		$this->db->where('tareas.gestion',$gestion);
		$this->db->where('temas.id_mat',$materia);
		$this->db->where('temas.id_prof',$id_prof);
	...
	function get_datatables_gestion($cod_curso,$cod_nivel,$gestion,$materia,$id_prof)
	{
		$this->_get_datatables_query_gestion($cod_curso,$cod_nivel,$gestion,$materia,$id_prof);
		if($_POST['length'] != -1)
		$this->db->limit($_POST['length'], $_POST['start']);
		$query = $this->db->get();
		return $query->result();
	...
	public function tarea_materia($gestion,$cod_curso,$cod_nivel)
	{
		$this->db->distinct();
		$this->db->select('materias.id_mat as id_materia,materias.nombre as materia');
		$this->db->from("tareas");
		$this->db->join('temas', 'temas.id = tareas.id_tema','inner');
		$this->db->join('materias', 'materias.id_mat = temas.id_mat','inner');
		$this->db->where('tareas.gestion',$gestion);
	...
	public function tarea_tema($gestion,$cod_curso,$cod_nivel,$materia)
	{
		$this->db->select('temas.id as id_tema,tareas.id as id_tarea,temas.tema,profesores.appaterno,profesores.apmaterno,profesores.nombre,tareas.fechaentrega as fecha');
		$this->db->from("tareas");
		$this->db->join('temas', 'temas.id = tareas.id_tema','inner');
		$this->db->join('materias', 'materias.id_mat = temas.id_mat','inner');
		$this->db->join('profesores', 'profesores.id_prof = temas.id_prof','inner');
		...
		$this->db->where('temas.cod_curso',$cod_curso);
		$this->db->where('temas.cod_nivel',$cod_nivel);
		$this->db->where('materias.id_mat',$materia);
		$query=$this->db->get();
		return $query->result();
	}
	...
	public function Estudiantes($id_est,$gestion)
	{
		$sql="SELECT CONCAT(estudiantes.appaterno ,' ',estudiantes.apmaterno,' ',estudiantes.nombre) as nombres,estudiantes.id_est,nota_prom.prom1 as final, estudiantes.appaterno , estudiantes.apmaterno, estudiantes.nombre
			...
			WHERE nota_prom.retirado=false AND nota_prom.gestion=".$gestion."  and nota_prom.id_est=".$id_est." 
	...
	public function get_subir_tarea($id)
	{
		$this->db->select("tarea_subir.*,temas.id_mat");
		$this->db->from("tarea_subir");
		$this->db->join('temas', 'temas.id = tarea_subir.id_tema','inner');
		$this->db->where('tarea_subir.id',$id);
		$query=$this->db->get();
		return $query->row();
	}
	public function verificar_tarea($id_est,$d_tema,$id_tarea,$gest)
	{
		$this->db->select('DATE_FORMAT(tarea_subir.fecha, "%H:%I:%S" )as hora,Date_format(tarea_subir.fecha,"%Y/%M/%d") as fechas,tarea_subir.nombre as archivo,tarea_subir.id');
		$this->db->from("tarea_subir");
		$this->db->where('id_est',$id_est);
		$this->db->where('gestion',$gest);
		$this->db->where('id_tema',$d_tema);
	...
	function count_filtered_gestion($cod_curso,$cod_nivel,$gestion,$materia,$id_prof)
	{
		$this->_get_datatables_query_gestion($cod_curso,$cod_nivel,$gestion,$materia,$id_prof);
		$query = $this->db->get();
		return $query->num_rows();
	...
	public function get_compromiso($id_est,$gestion)
	{
   ...
   function delete_compromiso($id_est,$gestion)
	{
		$this->db->where('id_est',$id_est);
		$this->db->where('gestion',$gestion);
		$this->db->delete("Compomiso");
	}
