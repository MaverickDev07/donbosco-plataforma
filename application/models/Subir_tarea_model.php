...
class Subir_tarea_model extends CI_Model {
 
	var $tabla='temas';
	var $column= array('temas.id','cursos.nombre','niveles.turno','niveles.nivel','temas.gestion','materias.nombre','temas.tema'); //set column field database for datatable orderable
	...
	var $order = array('temas.fecha' => 'asc'); // default order 
	...
	{		
		$this->db->select('tareas.id as id,cursos.nombre as curso,niveles.turno,niveles.nivel,temas.gestion,materias.nombre as materia,temas.tema as tema,tareas.nombre as archivo');
		$this->db->from("tareas");
	...
	{		
		$this->db->select('temas.id as id,cursos.nombre as curso,niveles.turno,niveles.nivel,temas.gestion,materias.nombre as materia,temas.tema as tema,temas.nombre as archivo');
		$this->db->from("temas");
		$this->db->join('profesores', 'profesores.id_prof = temas.id_prof','inner');
		$this->db->join('cursos', 'cursos.codigo = temas.cod_curso','inner');
		$this->db->join('niveles', 'niveles.codigo = temas.cod_nivel','inner');
	...
	public function get_nivel_profesor($id_prof,$gestion)
	{
		$this->db->distinct();
		$this->db->select("niveles.*");
	...
	public function get_curso_profesor($id_prof,$gestion,$nivele)
	{
		$this->db->distinct();
		...
		$this->db->where('asiginar_profesorm.id_prof', $id_prof);
		$this->db->where('asiginar_profesorm.gestion', $gestion);
		$this->db->where('niveles.codigo', $nivele);
		$query=$this->db->get();
		return $query->result();
	}
	public function get_materia_profesor($id_prof,$gestion,$nivele,$cod_curso)
	{
		$this->db->distinct();
		...
		$this->db->where('asiginar_profesorm.gestion', $gestion);
		$this->db->where('niveles.codigo', $nivele);
		$this->db->where('cursos.codigo', $cod_curso);
		$query=$this->db->get();
		return $query->result();
	}
	...
	/* New Functions */
	public function checkTareas($id_tema)
	{
		$sql = "SELECT * FROM tareas WHERE id_tema = $id_tema";
		$query=$this->db->query($sql);
		return $query->result();
	}
