...
class Profesor_link_model extends CI_Model {
 
	var $tabla='temas';  
	var $column= array('clase_virtual.id','materias.nombre','temas.tema'); //set column field database for datatable orderable
	...
	{		
		$this->db->select('clase_virtual.*,materias.nombre as materia,cursos.nombre as curso');
		$this->db->from("clase_virtual");
		$this->db->join('materias', 'materias.id_mat = clase_virtual.id_mat','inner');
		$this->db->join('cursos', 'cursos.codigo = clase_virtual.cod_curso','inner');
		$this->db->where('clase_virtual.gestion',$gestion);
		$this->db->where('clase_virtual.cod_nivel',$nivel);
		$this->db->where('clase_virtual.id_prof',$id_prof);
		$i = 0;
