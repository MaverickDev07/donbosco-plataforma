...
class Estudiantes_ca_model extends CI_Model {
  
	var $tabla='calendario';
	var $column= array('calendario.id .id','materias.nombre','profesores.appaterno','profesores.apmaterno','profesores.nombre,calendario.nombre'); //set column field database for datatable orderable
	...
	var $order1 = array('calendario.id_mat' => 'asc','calendario.mes' => 'asc'); 
	var $order = array('calendario.mes' => 'asc'); // default order 
	...
	private function _get_datatables_query_gestion1($curso,$nivel,$gestion)
	{		
		$this->db->select('calendario.id as id,materias.nombre as materia,profesores.appaterno,profesores.apmaterno,profesores.nombre, calendario.nombre as archivo, calendario.mes');
		$this->db->from("calendario");
		$this->db->join('profesores', 'profesores.id_prof = calendario.id_prof','inner');
		$this->db->join('materias', 'materias.id_mat = calendario.id_mat','inner');
		$this->db->where('calendario.gestion',$gestion);
		$this->db->where('calendario.cod_curso',$curso);
		$this->db->where('calendario.cod_nivel',$nivel);
	...
	private function _get_datatables_query_gestion($curso,$nivel,$gestion,$materia,$id_prof)
	{		
		$this->db->select('calendario.id as id,materias.nombre as materia,profesores.appaterno,profesores.apmaterno,profesores.nombre, calendario.nombre as archivo, calendario.mes');
		$this->db->from("calendario");
		$this->db->join('profesores', 'profesores.id_prof = calendario.id_prof','inner');
		...
		$this->db->where('calendario.cod_curso',$curso);
		$this->db->where('calendario.cod_nivel',$nivel);
		$this->db->where('calendario.id_mat',$materia);
		$this->db->where('calendario.id_prof',$id_prof);
	...
	function get_datatables_gestion($curso,$nivel,$gestion,$materia,$id_prof)
	{
		$this->_get_datatables_query_gestion($curso,$nivel,$gestion,$materia,$id_prof);
		if($_POST['length'] != -1)
		$this->db->limit($_POST['length'], $_POST['start']);
		$query = $this->db->get();
		return $query->result();
	...
	function get_datatables_gestion1($curso,$nivel,$gestion)
	{
		$this->_get_datatables_query_gestion1($curso,$nivel,$gestion);
		if($_POST['length'] != -1)
		$this->db->limit($_POST['length'], $_POST['start']);
		$query = $this->db->get();
		return $query->result();
	...
	public function get_temas_nombre($archivo)
	...
	function count_filtered_gestion($curso,$nivel,$gestion,$materia,$id_prof)
	{
		$this->_get_datatables_query_gestion($curso,$nivel,$gestion,$materia,$id_prof);
		$query = $this->db->get();
		return $query->num_rows();
	...
	function count_filtered_gestion1($curso,$nivel,$gestion)
	{
		$this->_get_datatables_query_gestion1($curso,$nivel,$gestion);
		$query = $this->db->get();
		return $query->num_rows();
