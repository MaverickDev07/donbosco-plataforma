...
class Profesor_de_model extends CI_Model {
 
	var $tabla='temas'; 
	var $column= array('tarea_subir.id','cursos.nombre','tarea_subir.fecha','materias.nombre','temas.tema', 'tarea_subir.nombre','estudiantes.appaterno','estudiantes.apmaterno','estudiantes.nombre'); //set column field database for datatable orderable
	...
	{		
		$this->db->select('tarea_subir.id as id,cursos.nombre as curso,  DATE_FORMAT(tarea_subir.fecha, "%H:%I:%S" )as hora,Date_format(tarea_subir.fecha,"%Y/%M/%d") as fecha,materias.nombre as materia,temas.tema as tema, tarea_subir.nombre as archivo,estudiantes.appaterno,estudiantes.apmaterno,estudiantes.nombre,tarea_subir.estado');
		$this->db->from("tarea_subir");
		$this->db->join('temas', 'temas.id = tarea_subir.id_tema','inner');
		$this->db->join('cursos', 'cursos.codigo = temas.cod_curso','inner');
		$this->db->join('materias', 'materias.id_mat = temas.id_mat','inner');
		$this->db->join('estudiantes', 'tarea_subir.id_est = estudiantes.id_est','inner');
		$this->db->where('temas.gestion',$gestion);
